...
use crate::config::{StorageConfig, VialDeviceSettingsData};
...
pub(crate) enum FlashOperationMessage {
    ...
    DeviceSettings(VialDeviceSettingsData),
}
...
impl StorageKeys {
    pub(crate) fn from_u8(value: u8) -> Option<Self> {
        match value {
            ...
            #[cfg(feature = "host")]
            10 => Some(StorageKeys::DeviceSettings),
            #[cfg(all(feature = "_ble", feature = "split"))]
...
impl StorageData {
    ...
    const fn key(&self) -> u32 {
        match self {
            ...
            #[cfg(feature = "host")]
            Self::DeviceSettings(_) => StorageKeys::DeviceSettings as u32,
        }
    }
}
...
impl Value<'_> for StorageData {
    fn serialize_into(&self, buffer: &mut [u8]) -> Result<usize, SerializationError> {
        ...
        match self {
            ...
            #[cfg(feature = "host")]
            Self::DeviceSettings(d) => ser_storage_variant!(buffer, StorageKeys::DeviceSettings, d),
        }
    }
        ...
        Self: Sized,
    {
        ...
        match key {
            ...
            #[cfg(feature = "host")]
            StorageKeys::DeviceSettings => {
                let (data, unused) =
                    postcard::take_from_bytes(&buffer[1..]).map_err(postcard_error_to_serialization_error)?;
                let size = buffer.len() - unused.len();
                Ok((Self::DeviceSettings(data), size))
            }
        }
    }
}
    ...
    Storage<F, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    pub async fn new(
        ...
        } else if storage_config.clear_layout && storage.stored_build_hash_changed().await {
            #[cfg(feature = "host")]
            {
                debug!("clear_layout=true and build hash changed; overwriting layout items without erase.");
                ...
                if storage
                    ...
                    .is_ok()
                {
                    let _ = storage.write_storage_config_enabled().await;
                }
            }
        }
            ...
            match match info {
                ...
                FlashOperationMessage::DeviceSettings(data) => {
                    store_item(
                        ...
                        &mut storage_cache,
                        &mut self.buffer,
                        &(StorageKeys::DeviceSettings as u32),
    ...
    #[cfg(feature = "host")]
    pub(crate) async fn read_device_settings(&mut self) -> Result<Option<VialDeviceSettingsData>, ()> {
        if let Some(StorageData::DeviceSettings(data)) = fetch_item::<u32, StorageData, _>(
            &mut self.flash,
            self.storage_range.clone(),
            &mut NoCache::new(),
            &mut self.buffer,
    ...
    async fn stored_build_hash_changed(&mut self) -> bool {
        if let Ok(Some(StorageData::StorageConfig(config))) = fetch_item::<u32, StorageData, _>(
            &mut self.flash,
            self.storage_range.clone(),
            &mut NoCache::new(),
        ...
        .await
        {
            return config.build_hash != BUILD_HASH;
        }
        true
    }

    async fn write_storage_config_enabled(&mut self) -> Result<(), SSError<F::Error>> {
        store_item(
            &mut self.flash,
            self.storage_range.clone(),
            &mut NoCache::new(),
