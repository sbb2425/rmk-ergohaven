...
static BATTERY_ADC_FORCE_SAMPLE: Signal<crate::RawMutex, ()> = Signal::new();

pub fn request_battery_adc_sample() {
    BATTERY_ADC_FORCE_SAMPLE.signal(());
}
...
impl<'a, const PIN_NUM: usize, const EVENT_NUM: usize> NrfAdc<'a, PIN_NUM, EVENT_NUM> {
    ...
    fn has_battery_event(&self) -> bool {
        for event in &self.event_type {
            if matches!(event, AnalogEventType::Battery) {
                return true;
            }
        }
        false
    ...
    async fn wait_next_poll(&self, delay: Duration) {
        if self.has_battery_event() {
            let _ = select(Timer::after(delay), BATTERY_ADC_FORCE_SAMPLE.wait()).await;
        } else {
        ...
        if self.active_instant == Instant::MIN {
            ...
            if let Some(light_sleep) = self.light_sleep {
                if self.adc_state == AdcState::LightSleep {
                    self.wait_next_poll(light_sleep).await;
                } else {
                    self.wait_next_poll(self.polling_interval).await;
                }
            } else {
                self.wait_next_poll(self.polling_interval).await;
            }
