...
#[cfg(all(feature = "controller", feature = "_ble", feature = "split"))]
use crate::{channel::send_controller_event_new, event::ControllerEvent};
...
const ERGOHAVEN_CUSTOM_VALUE: u8 = 0xe8;
const ERGOHAVEN_BATTERY_HALVES: u8 = 0x01;
    ...
    async fn process_via_packet(
        ...
        match command_id.into() {
            ...
            ViaCommand::CustomGetValue => {
                match (report.output_data[1], report.output_data[2]) {
                    (ERGOHAVEN_CUSTOM_VALUE, ERGOHAVEN_BATTERY_HALVES) => {
                        #[cfg(feature = "_nrf_ble")]
                        crate::input_device::adc::request_battery_adc_sample();
                        #[cfg(all(feature = "controller", feature = "_ble", feature = "split"))]
                        send_controller_event_new(ControllerEvent::SplitBatteryLevelRequest(0));

                        let (left, right) = crate::input_device::battery::battery_halves_percent();
                        let mut flags = 0u8;
                        if left != crate::input_device::battery::BATTERY_PERCENT_UNKNOWN {
                            flags |= 1 << 0;
                        }
                        if right != crate::input_device::battery::BATTERY_PERCENT_UNKNOWN {
                            flags |= 1 << 1;
                        }

                        report.input_data[1] = ERGOHAVEN_CUSTOM_VALUE;
                        report.input_data[2] = ERGOHAVEN_BATTERY_HALVES;
                        report.input_data[3] = ERGOHAVEN_BATTERY_HALVES_VERSION;
                        report.input_data[4] = flags;
                        report.input_data[5] = left;
                        report.input_data[6] = right;
                    }
                    _ => {
                        // backlight/rgblight/rgb matrix/led matrix/audio settings here
            ...
            ViaCommand::EepromReset => {
                ...
                #[cfg(all(feature = "controller", feature = "_ble", feature = "split"))]
                send_controller_event_new(ControllerEvent::ClearPeer);
                #[cfg(feature = "storage")]
