...
pub(crate) enum SplitMessage {
    ...
    Layer(u8),
    /// Device settings blob from central to peripheral
    DeviceSettings([u8; 27]),
    /// Request a one-shot local battery level indication
    BatteryLevelRequest,
    /// Latest split peripheral battery percentage
    BatteryLevel(u8),
    /// Product id handshake used to reject peripherals from another keyboard model.
    ProductId(u16),
}
