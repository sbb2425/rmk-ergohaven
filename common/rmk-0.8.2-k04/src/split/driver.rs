    ...
    PeripheralManager<ROW, COL, ROW_OFFSET, COL_OFFSET, T>
{
    ...
    pub(crate) async fn run(mut self) -> bool {
        ...
        if let Err(e) = self.transceiver.write(&SplitMessage::ConnectionState(conn_state)).await {
            match e {
                SplitDriverError::Disconnected => return false,
        ...
        loop {
            ...
            .await
            {
                    ...
                    match controller_msg {
                        ...
                        ControllerEvent::ClearPeer => {
                            ...
                            if let Err(e) = self.transceiver.write(&SplitMessage::ClearPeer).await {
                                match e {
                                    SplitDriverError::Disconnected => return true,
                                    _ => error!("SplitDriver write error: {:?}", e),
                                }
                            }
                            return true;
                        ...
                        ControllerEvent::DeviceSettings(settings) => {
                            debug!("Sending device settings to peripheral {}", self.id);
                            if let Err(e) = self.transceiver.write(&SplitMessage::DeviceSettings(settings)).await {
                                match e {
                                    SplitDriverError::Disconnected => return false,
                                    _ => error!("SplitDriver write error: {:?}", e),
                                }
                        ...
                        ControllerEvent::SplitBatteryLevelRequest(id) if id == self.id => {
                            debug!("Sending battery level request to peripheral {}", self.id);
                            if let Err(e) = self.transceiver.write(&SplitMessage::BatteryLevelRequest).await {
                                match e {
                                    SplitDriverError::Disconnected => return false,
                                    _ => error!("SplitDriver write error: {:?}", e),
                                }
                ...
                Either3::Second(event) => match event {
                    ...
                    _ => send_latest_non_key_event(event),
                },
                Either3::Third(_) => {
...
fn send_latest_non_key_event(event: Event) {
    match EVENT_CHANNEL.try_send(event) {
        Ok(()) => {}
        Err(crate::channel::channel::TrySendError::Full(event)) => {
        ...
        loop {
            match self.transceiver.read().await {
                ...
                Ok(SplitMessage::BatteryLevel(level)) => {
                    crate::input_device::battery::set_remote_battery_percent(level);
                }
                Ok(_) => {
